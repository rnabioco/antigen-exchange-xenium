...
GiottoClass::set_giotto_python_path("umap-2")
...
meta <- xen %>%
  filter(slide == "section 1") %>%
  select(cell_ID = cell_id, cell_type, Ag_class) %>%
  mutate(cell_type = str_c(cell_type, " (", Ag_class, ")")) %>%
  select(-Ag_class) %>%
  readCellMetadata()
...
obj %>%
  qsave(here("results/net_gio.qs"))

obj <- qread(here("results/net_gio.qs"))
